...
We remove undrafted bc only 9 have ever broken out and only 1 since 2019 and the other 8 were before 2016.
...
```{r}
last_rf_mod <- 
  rand_forest(mtry = 335, min_n = 32, trees = 1500) %>%
...
```{r}
table(knn_pred$breakout,knn_pred$.pred_class)
```
...
```{r}
table(svm_pred$breakout,svm_pred$.pred_class)
```
