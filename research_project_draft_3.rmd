...
```{r}
updated_final <- final %>% 
  mutate(updated_b_o = if_else(.pred_Yes > .35, "Breakout", "No Breakout"))
updated_final
```

```{r fig.height=12, fig.width=10}
ggplot(updated_final) +
  aes(x = reorder(player, .pred_Yes), y = .pred_Yes) + 
  geom_col() +
  geom_text(aes(label = round(.pred_Yes,2), hjust = -.20)) +
  coord_flip()
```


```{r}
table(updated_final$breakout,updated_final$updated_b_o)
```