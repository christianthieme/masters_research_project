...
```{r}
# wr <- readr::read_csv("C:/Users/chris/OneDrive/Junk/test1.csv")  %>% 
#   mutate_if(is.character, as.factor) %>% 
#   filter(Player == 'Calvin Johnson') %>% 
# 
# wr
```
...
```{r}
wr2 <- wr %>% 
  select('Player', 'School', 'Conf','DOB', 'DR', 'DP','Draft_Year', 'Draft_Age',  'BreakoutAge_WRBOA_twentypercent','BreakoutAge_WRBOA_thirtypercent','TotalCareer_REC', 'GamesPlayed_18','GamesPlayed_19','GamesPlayed_20', 'GamesPlayed_21', 'GamesPlayed_22',  "RushATTs_18" ,                         
...
```
```{r}
wr2 %>% filter(Player == 'Calvin Johnson')
```
...
```{r}
last_rf_mod <- 
  rand_forest(mtry = 1, min_n = 27, trees = 1500) %>%
...
```{r}
table(fitted$breakout,fitted$.pred_class)
```
...
```{r}
final %>% 
  filter(.pred_Yes < 0.50 & breakout == 'Yes')
```
```{r}
final %>% 
  filter(.pred_Yes > 0.50 & breakout == 'No')
```
...
```{r}
table(xg_pred$breakout,xg_pred$.pred_class)
```

```{r}
ensemble <- xg_pred %>% 
  left_join(predictions[c(".row",".pred_No", ".pred_Yes")], by = ".row") %>%
  mutate(pred_No = (.pred_No.x + .pred_No.y)/2) %>%
  mutate(pred_Yes = (.pred_Yes.x + .pred_Yes.y)/2) %>%
  mutate(new_breakout = ifelse(pred_Yes > 0.5, "Yes", "No"))
```

```{r}
table(ensemble$breakout, ensemble$new_breakout)
```
